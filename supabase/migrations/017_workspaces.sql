create table if not exists public.enrichment_folders (
...
create unique index if not exists enrichment_folders_name_lower_key on public.enrichment_folders (lower(name));

create table if not exists public.enrichment_workbooks (
  id uuid primary key default gen_random_uuid(),
  slug text not null unique,
  name text not null check (char_length(name) <= 80),
  description text not null default '' check (char_length(description) <= 300),
  folder_id uuid references public.enrichment_folders(id) on delete restrict,
  sort_order integer not null default 0,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create unique index if not exists enrichment_workbooks_name_lower_key on public.enrichment_workbooks (lower(name));

create table if not exists public.enrichment_tables (
  id uuid primary key default gen_random_uuid(),
  workbook_id uuid not null references public.enrichment_workbooks(id) on delete restrict,
  slug text not null,
  name text not null check (char_length(name) <= 80),
  description text not null default '' check (char_length(description) <= 300),
  source text not null default 'leads' check (source = 'leads'),
...
create unique index if not exists enrichment_tables_workbook_name_lower_key on public.enrichment_tables (workbook_id, lower(name));

create table if not exists public.enrichment_table_exports (
  id uuid primary key default gen_random_uuid(),
  ...
  unique (table_id, lead_id, campaign_id)
);

create index if not exists enrichment_table_exports_table_lead_idx on public.enrichment_table_exports (table_id, lead_id);

alter table public.enrichment_folders enable row level security;
alter table public.enrichment_workbooks enable row level security;
alter table public.enrichment_tables enable row level security;
alter table public.enrichment_table_exports enable row level security;

revoke all on table public.enrichment_folders from anon, authenticated;
revoke all on table public.enrichment_workbooks from anon, authenticated;
revoke all on table public.enrichment_tables from anon, authenticated;
revoke all on table public.enrichment_table_exports from anon, authenticated;

grant select, insert, update, delete on table public.enrichment_folders to service_role;
grant select, insert, update, delete on table public.enrichment_workbooks to service_role;
grant select, insert, update, delete on table public.enrichment_tables to service_role;
grant select, insert, update, delete on table public.enrichment_table_exports to service_role;

create or replace function public.enrichment_set_updated_at()
...
revoke execute on function public.enrichment_set_updated_at() from public, anon, authenticated;
grant execute on function public.enrichment_set_updated_at() to service_role;

drop trigger if exists enrichment_folders_updated_at on public.enrichment_folders;
create trigger enrichment_folders_updated_at before update on public.enrichment_folders for each row execute function public.enrichment_set_updated_at();
drop trigger if exists enrichment_workbooks_updated_at on public.enrichment_workbooks;
create trigger enrichment_workbooks_updated_at before update on public.enrichment_workbooks for each row execute function public.enrichment_set_updated_at();
drop trigger if exists enrichment_tables_updated_at on public.enrichment_tables;
create trigger enrichment_tables_updated_at before update on public.enrichment_tables for each row execute function public.enrichment_set_updated_at();

insert into public.enrichment_workbooks (slug, name, description)
values ('manufacturing-ops', 'Manufacturing Ops', 'Operations leaders at ICP manufacturers across the US, UK, Canada, and Australia.')
on conflict do nothing;

insert into public.enrichment_tables (workbook_id, slug, name, description, config)
select w.id, 'two-contact-leads', 'Two-contact send list', 'ICP companies with at least two named contacts for coworker-based personalization.', '{}'::jsonb
from public.enrichment_workbooks w
where w.slug = 'manufacturing-ops'
  and not exists (
    select 1 from public.enrichment_tables t where t.workbook_id = w.id and t.slug = 'two-contact-leads'
  );

insert into public.enrichment_table_exports (table_id, lead_id, campaign_id, campaign_name, status, error, exported_at)
select t.id, l.id, l.smartlead_campaign_id, l.smartlead_campaign_name, l.smartlead_export_status, l.smartlead_export_error,
  coalesce(l.smartlead_exported_at, now())
from public.leads l
cross join public.enrichment_workbooks w
join public.enrichment_tables t on t.workbook_id = w.id and t.slug = 'two-contact-leads'
where w.slug = 'manufacturing-ops'
  and l.smartlead_export_status in ('exported', 'failed')
  and l.smartlead_campaign_id is not null
on conflict (table_id, lead_id, campaign_id) do nothing;

update public.enrichment_views v
set table_id = t.id::text
from public.enrichment_workbooks w
join public.enrichment_tables t on t.workbook_id = w.id and t.slug = 'two-contact-leads'
where w.slug = 'manufacturing-ops' and v.table_id = 'leads';

delete from public.enrichment_views
where table_id !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$';

alter table public.enrichment_views drop constraint if exists enrichment_views_table_id_fkey;
alter table public.enrichment_views alter column table_id type uuid using table_id::uuid;
alter table public.enrichment_views add constraint enrichment_views_table_id_fkey foreign key (table_id) references public.enrichment_tables(id) on delete cascade;

drop function if exists public.enrichment_leads_page(integer, integer, text, text[], text[], text[], boolean, boolean, text, text);
...
begin
  ...
  with filtered as (
    ...
    where l.send_wave is not null
      and l.company_id in (select x.company_id from public.leads x where x.send_wave is not null group by x.company_id having count(*) >= 2)
...
drop function if exists public.enrichment_run_candidates(text, text, integer, timestamptz);
...
begin
  ...
  from public.leads l
  where l.send_wave is not null and l.company_id in (select x.company_id from public.leads x where x.send_wave is not null group by x.company_id having count(*)>=2)
    and (p_role_levels is null or cardinality(p_role_levels)=0 or coalesce(l.role_level,'unknown')=any(p_role_levels))
    and (p_email_statuses is null or cardinality(p_email_statuses)=0 or coalesce(l.email_status,'unknown')=any(p_email_statuses))
    and (p_countries is null or cardinality(p_countries)=0 or coalesce(nullif(btrim(coalesce(l.raw->>'Lead Country','')),''),'unknown')=any(p_countries))
    and (p_has_email is null or p_has_email=(nullif(btrim(l.email),'') is not null))
...
create or replace function public.enrichment_swap_table_sort_orders(p_table_id uuid, p_neighbor_id uuid)
returns void language plpgsql security invoker set search_path = '' as $$
declare
  v_table_order integer;
  v_neighbor_order integer;
  v_table_workbook uuid;
  v_neighbor_workbook uuid;
begin
  select t.sort_order,t.workbook_id into v_table_order,v_table_workbook from public.enrichment_tables t where t.id=p_table_id for update;
  select t.sort_order,t.workbook_id into v_neighbor_order,v_neighbor_workbook from public.enrichment_tables t where t.id=p_neighbor_id for update;
  if v_table_workbook is null or v_neighbor_workbook is null or v_table_workbook <> v_neighbor_workbook then
    raise exception 'Tables must exist in the same workbook to reorder';
  end if;
  update public.enrichment_tables set sort_order=case when id=p_table_id then v_neighbor_order else v_table_order end where id in (p_table_id,p_neighbor_id);
end;
$$;

revoke execute on function public.enrichment_leads_page(integer,integer,text,text[],text[],text[],boolean,boolean,text,text,uuid) from public, anon, authenticated;
revoke execute on function public.enrichment_run_candidates(text,text,integer,timestamptz,text[],text[],text[],boolean,boolean,uuid) from public, anon, authenticated;
revoke execute on function public.enrichment_swap_table_sort_orders(uuid,uuid) from public, anon, authenticated;
grant execute on function public.enrichment_leads_page(integer,integer,text,text[],text[],text[],boolean,boolean,text,text,uuid) to service_role;
grant execute on function public.enrichment_run_candidates(text,text,integer,timestamptz,text[],text[],text[],boolean,boolean,uuid) to service_role;
grant execute on function public.enrichment_swap_table_sort_orders(uuid,uuid) to service_role;
