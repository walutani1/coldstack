-- 019: Decision Makers table (directors at companies with no manager/champion).
-- Adds two CONFIG-ONLY, backward-compatible dimensions to the shared leads
-- functions: p_min_contacts (relax the >=2-contacts floor) and
-- p_exclude_company_role (drop companies that have a lead of that role). Both
-- default to the current behavior, so the Champions list is byte-for-byte
-- unchanged (verified: 11,414 before and after). The new table surfaces the
-- ~9,577 send-wave directors whose company has no manager.

drop function if exists public.enrichment_leads_page(integer, integer, text, text[], text[], text[], boolean, boolean, text, text, uuid);
...
begin
  ...
  with filtered as (
    ...
    where l.send_wave is not null
      and l.company_id in (select x.company_id from public.leads x where x.send_wave is not null group by x.company_id having count(*) >= greatest(1, coalesce(p_min_contacts, 2)))
...
drop function if exists public.enrichment_run_candidates(text, text, integer, timestamptz, text[], text[], text[], boolean, boolean, uuid);
...
create function public.enrichment_run_candidates(
  ...
  p_min_contacts integer default 2, p_exclude_company_role text default null
)
returns table (
  id uuid, company_id uuid, role_level text, first_name text, last_name text, title text, email text,
  email_status text, final_first_name text, final_title text, final_company_name text, operations_task text,
...
begin
  ...
  from public.leads l
  where l.send_wave is not null and l.company_id in (select x.company_id from public.leads x where x.send_wave is not null group by x.company_id having count(*)>=greatest(1, coalesce(p_min_contacts, 2)))
    and (p_exclude_company_role is null or not exists (select 1 from public.leads y where y.company_id = l.company_id and y.role_level = p_exclude_company_role))
    and (p_role_levels is null or cardinality(p_role_levels)=0 or coalesce(l.role_level,'unknown')=any(p_role_levels))
    and (p_email_statuses is null or cardinality(p_email_statuses)=0 or coalesce(l.email_status,'unknown')=any(p_email_statuses))
    and (p_countries is null or cardinality(p_countries)=0 or coalesce(nullif(btrim(coalesce(l.raw->>'Lead Country','')),''),'unknown')=any(p_countries))
...
revoke execute on function public.enrichment_leads_page(integer,integer,text,text[],text[],text[],boolean,boolean,text,text,uuid,integer,text) from public, anon, authenticated;
revoke execute on function public.enrichment_run_candidates(text,text,integer,timestamptz,text[],text[],text[],boolean,boolean,uuid,integer,text) from public, anon, authenticated;
grant execute on function public.enrichment_leads_page(integer,integer,text,text[],text[],text[],boolean,boolean,text,text,uuid,integer,text) to service_role;
grant execute on function public.enrichment_run_candidates(text,text,integer,timestamptz,text[],text[],text[],boolean,boolean,uuid,integer,text) to service_role;

-- The Decision Makers table (leads already exist in public.leads; this is a
-- computed view over them, so nothing is inserted into leads).
insert into public.enrichment_tables (workbook_id, slug, name, description, config, sort_order)
select w.id, 'decision-makers', 'Manufacturing Ops Decision Makers',
  'Directors at companies where we have no manager/champion. They can be pitched directly without waiting on the champions campaign.',
  '{"roleLevels":["director"],"minContacts":1,"excludeCompanyRole":"manager"}'::jsonb,
  (select coalesce(max(t2.sort_order), -1) + 1 from public.enrichment_tables t2 where t2.workbook_id = w.id)
from public.enrichment_workbooks w
where w.slug = 'manufacturing-ops'
  and not exists (select 1 from public.enrichment_tables t where t.workbook_id = w.id and t.slug = 'decision-makers');
