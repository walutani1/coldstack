drop function if exists public.crm_leads_page(integer,integer,text,text[],boolean,text[],text[],boolean,boolean,text[],boolean,text[],timestamptz,timestamptz,timestamptz,timestamptz,text,text,text[]);
    ...
    where (p_search is null or btrim(p_search)='' or
      ...
      and (p_campaigns_exclude is null or cardinality(p_campaigns_exclude)=0 or (
        ...
        and not exists (
          ...
          where rc.lead_id=d.id and rc.campaign_id=any(p_campaigns_exclude)
        )
      ))
