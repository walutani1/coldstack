create table if not exists public.untracked_replies (
  ...
  status text not null default 'new'
    check (status in ('new','processing','automated','duplicate','suggested','possible_match','unmatched','importing','imported','dismissed','error')),
  ...
  proposed_contact jsonb,
  attempt_count integer not null default 0,
  process_started_at timestamptz,
  next_attempt_at timestamptz,
  last_error text,
  imported_reply_event_id uuid,
  reviewed_by text,
...
create unique index if not exists untracked_replies_imported_event_uidx
  on public.untracked_replies(imported_reply_event_id) where imported_reply_event_id is not null;
create index if not exists untracked_replies_work_idx
  on public.untracked_replies(status, next_attempt_at, first_seen_at);
create index if not exists untracked_replies_from_email_idx
  on public.untracked_replies(lower(from_email));

create table if not exists public.untracked_reply_sync_state (
  provider text primary key,
  next_offset integer not null default 500,
...
alter table public.reply_events add column if not exists source text not null default 'smartlead_webhook';
alter table public.reply_events add column if not exists source_ref uuid;
alter table public.reply_events add column if not exists metric_eligible boolean not null default true;

create unique index if not exists reply_events_untracked_source_ref_uidx
  on public.reply_events(source_ref) where source='smartlead_untracked_import';

alter table public.untracked_replies
  drop constraint if exists untracked_replies_imported_reply_event_id_fkey;
alter table public.untracked_replies
  add constraint untracked_replies_imported_reply_event_id_fkey
  foreign key (imported_reply_event_id) references public.reply_events(id) on delete set null;

create or replace function public.untracked_replies_set_updated_at()
...
revoke execute on function public.untracked_replies_set_updated_at() from public, anon, authenticated;
grant execute on function public.untracked_replies_set_updated_at() to service_role;

drop trigger if exists untracked_replies_updated_at on public.untracked_replies;
create trigger untracked_replies_updated_at before update on public.untracked_replies
for each row execute function public.untracked_replies_set_updated_at();
drop trigger if exists untracked_reply_sync_state_updated_at on public.untracked_reply_sync_state;
create trigger untracked_reply_sync_state_updated_at before update on public.untracked_reply_sync_state
for each row execute function public.untracked_replies_set_updated_at();

alter table public.untracked_replies enable row level security;
alter table public.untracked_reply_sync_state enable row level security;
revoke all on table public.untracked_replies from public, anon, authenticated;
revoke all on table public.untracked_reply_sync_state from public, anon, authenticated;
grant select, insert, update, delete on table public.untracked_replies to service_role;
grant select, insert, update, delete on table public.untracked_reply_sync_state to service_role;
...
begin
  ...
  select count(*) into v_count from public.reply_events e
  left join lateral (
...
drop function if exists public.crm_leads_page(integer,integer,text,text[],text[],boolean,text[],text[],boolean,boolean,text[],boolean,text[],timestamptz,timestamptz,timestamptz,timestamptz,text,text,text[]);
...
begin
  ...
  with members as (
    ...
    where exists (select 1 from public.enrichment_table_exports x where x.lead_id=l.id and x.status='exported')
       ...
       or exists (select 1 from public.reply_events re where re.lead_id=l.id and re.metric_eligible)
  ), decorated as (
    select m.*,
    ...
    left join lateral (
      ...
      from public.reply_events r where r.lead_id=m.id and r.metric_eligible
