-- 024: LinkedIn employment verification (Apify) as the FIRST enrichment gate.
-- Adds result columns to public.leads, returns the light status columns from
-- enrichment_leads_page (the jsonb payload is fetched on click, not per row),
-- and teaches enrichment_run_candidates the new 'linkedin_verify' action plus a
-- gate: a lead confirmed 'not_working' is skipped by EVERY other action, so we
-- never pay LeadMagic/ZeroBounce/LLM on someone who has left the company.
-- Backward compatible: the status is null for all existing leads, so the gate
-- (coalesce(...,'') <> 'not_working') excludes nothing until a check runs.
...
alter table public.leads drop constraint if exists leads_linkedin_employment_status_chk;
alter table public.leads add constraint leads_linkedin_employment_status_chk
  check (linkedin_employment_status is null or linkedin_employment_status in ('working','not_working','uncertain'));

drop function if exists public.enrichment_leads_page(integer,integer,text,text[],text[],text[],boolean,boolean,text,text,uuid,integer,text,boolean);
...
returns table (
  ...
  suppression_reason text, linkedin_employment_status text, linkedin_current_company text, linkedin_checked_at timestamptz, updated_at timestamptz, total_count bigint, table_export_status text,
...
drop function if exists public.enrichment_run_candidates(text,text,integer,timestamptz,text[],text[],text[],boolean,boolean,uuid,integer,text,boolean);
...
language plpgsql security invoker set search_path = '' as $$
begin
  if p_action not in ('find_email','validate_email','final_first_name','final_title','final_company_name','operations_task','ops_candidate','smartlead_export','linkedin_verify') then raise exception 'Unsupported enrichment action: %',p_action; end if;
  ...
  where (not coalesce(p_require_send_wave, true) or l.send_wave is not null) and l.company_id in (select x.company_id from public.leads x where (not coalesce(p_require_send_wave, true) or x.send_wave is not null) group by x.company_id having count(*)>=greatest(1, coalesce(p_min_contacts, 2)))
    and (p_exclude_company_role is null or not exists (select 1 from public.leads y where y.company_id = l.company_id and y.role_level = p_exclude_company_role))
    and (p_action='linkedin_verify' or coalesce(l.linkedin_employment_status,'') <> 'not_working')
    and (p_role_levels is null or cardinality(p_role_levels)=0 or coalesce(l.role_level,'unknown')=any(p_role_levels))
    and (p_email_statuses is null or cardinality(p_email_statuses)=0 or coalesce(l.email_status,'unknown')=any(p_email_statuses))
    and (p_countries is null or cardinality(p_countries)=0 or coalesce(nullif(btrim(coalesce(l.raw->>'Lead Country','')),''),'unknown')=any(p_countries))
    and (p_has_email is null or p_has_email=(nullif(btrim(l.email),'') is not null))
    ...
    and (case
      ...
      when p_action='linkedin_verify' then nullif(btrim(l.linkedin_url),'') is not null and (p_mode='force' or l.linkedin_employment_status is null)
