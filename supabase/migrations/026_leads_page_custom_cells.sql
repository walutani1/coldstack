-- 026: surface custom AI cell values + company_summary from enrichment_leads_page.
-- Purely additive: adds two return columns (company_summary jsonb, custom_cells
-- jsonb) to the page RPC. custom_cells is a {column key -> value} object built
-- from enrichment_cell_values for the requested table's columns; it is null for
-- any table that has no custom columns (e.g. Champions), so nothing changes for
-- existing lists. run_candidates is untouched. The client uses a passthrough
-- schema, so the extra columns are ignored until the v2 UI reads them.
...
returns table (
  ...
  suppression_reason text, linkedin_employment_status text, linkedin_current_company text, linkedin_checked_at timestamptz, company_summary jsonb, custom_cells jsonb, updated_at timestamptz, total_count bigint, table_export_status text,
