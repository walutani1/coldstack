-- Add custom_cell_gens to enrichment_leads_page: per-key generation_version for
-- each custom cell, so the client can detect a stale cell (generation older than
-- the column's) after a prompt edit and re-run it in the autorun.
...
-- Restore the same execute privileges the recreated function's DROP dropped.
