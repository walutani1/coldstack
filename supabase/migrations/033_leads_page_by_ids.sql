-- Add optional p_lead_ids to enrichment_leads_page so the client can fetch the
-- full normalized run-state for specific candidate leads (autorun waterfall).
...
revoke execute on function public.enrichment_leads_page(integer,integer,text,text[],text[],text[],boolean,boolean,text,text,uuid,integer,text,boolean,uuid[]) from public, anon, authenticated;
grant execute on function public.enrichment_leads_page(integer,integer,text,text[],text[],text[],boolean,boolean,text,text,uuid,integer,text,boolean,uuid[]) to service_role;
