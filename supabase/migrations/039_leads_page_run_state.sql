-- 039: Run-state filtering for the worktable.
--
-- Lets the UI filter the whole table (not just loaded rows) by a custom column's
-- cell state - not run / done / outdated - and by "errored in the most recent
-- run". Cell state is evaluated against enrichment_cell_values for the chosen
-- column; "errored" against that run job's failed items. All params default NULL
-- (no filter), so existing callers are unaffected.

DROP FUNCTION IF EXISTS public.enrichment_leads_page(integer,integer,text,text[],text[],text[],boolean,boolean,text,text,uuid,integer,text,boolean,uuid[]);

CREATE OR REPLACE FUNCTION public.enrichment_leads_page(p_limit integer, p_offset integer, p_search text, p_role_levels text[], p_email_statuses text[], p_countries text[], p_has_email boolean, p_qualified_only boolean, p_sort text, p_sort_dir text, p_table_id uuid DEFAULT NULL::uuid, p_min_contacts integer DEFAULT 2, p_exclude_company_role text DEFAULT NULL::text, p_require_send_wave boolean DEFAULT true, p_lead_ids uuid[] DEFAULT NULL::uuid[], p_cell_column_id uuid DEFAULT NULL::uuid, p_cell_state text DEFAULT NULL::text, p_cell_generation integer DEFAULT NULL::integer, p_errored_job_id uuid DEFAULT NULL::uuid)
...
revoke execute on function public.enrichment_leads_page(integer,integer,text,text[],text[],text[],boolean,boolean,text,text,uuid,integer,text,boolean,uuid[],uuid,text,integer,uuid) from public, anon, authenticated;
grant execute on function public.enrichment_leads_page(integer,integer,text,text[],text[],text[],boolean,boolean,text,text,uuid,integer,text,boolean,uuid[],uuid,text,integer,uuid) to service_role;
